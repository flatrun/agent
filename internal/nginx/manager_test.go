...
func TestGroupDomainsByHost_DeduplicatesLocations(t *testing.T) {
	...
	domains := []models.DomainConfig{
		...
		{ID: "4", Domain: "other.com", PathPrefix: "", ContainerPort: 80, Service: "other"},
	}

	servers := m.groupDomainsByHost(domains, "test-app")

	var exampleServer *serverData
	for i := range servers {
		if servers[i].Domain == "example.com" {
			exampleServer = &servers[i]
			break
		}
	}

	if exampleServer == nil {
		t.Fatal("expected to find server for example.com")
	}

	if len(exampleServer.Locations) != 2 {
		t.Errorf("expected 2 unique locations for example.com, got %d", len(exampleServer.Locations))
	}

	pathCounts := make(map[string]int)
	for _, loc := range exampleServer.Locations {
		pathCounts[loc.Path]++
	}
	...
	if pathCounts["/api"] != 1 {
		t.Errorf("expected exactly 1 location for '/api', got %d", pathCounts["/api"])
	}
}
...
func TestIsNginxConfigValid(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			output: "2026/02/03 17:33:27 [warn] 2572#2572: \"ssl_stapling\" ignored, no OCSP responder URL in the certificate \"/etc/letsencrypt/live/api.flatrun.dev/fullchain.pem\"\nnginx: [warn] \"ssl_stapling\" ignored\nnginx: the configuration file /etc/nginx/nginx.conf syntax is ok\nnginx: configuration file /etc/nginx/nginx.conf test is successful",
			want:   true,
		},
		{
			name:   "valid config with multiple warnings",
		...
		{
			name:   "error with success indicator should fail",
			output: "nginx: [emerg] something bad\nnginx: the configuration file /etc/nginx/nginx.conf syntax is ok",
			want:   false,
		},
	}
