...
- **Built-in observability**: per-container metrics are collected as time series using OpenTelemetry container semantic conventions and served for scraping in Prometheus format, so Grafana, SigLens, SigNoz or anything else that scrapes reads exactly what the built-in UI draws. No separate collector to run, and no lock-in on your own numbers.
